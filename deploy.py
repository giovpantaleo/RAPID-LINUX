...
default_epc_image = "epc-image-v1.ndz"#"oai-ci-cd-u18-epc-latest"
default_enb_image = "eNB-image-v1.ndz"#"oai-ci-u18-lowlatency-enb-ue-docker-latest"
default_quectel_image = "ue-image-v1.ndz"#"ue-quic32.llc"#"u20.04-quectel"
default_rapid_image ="rapid-image-v1.ndz"#"_RAPID-eki26-minRTT-2Harq.v1"#"ubuntu-18-boot-4.10-generic"#"u18-lowlat-rabbitmq"#"_RAPID-eki26-minRTT-2Harq.v1"#"u18-lowlat-rabbitmq"
default_server_image = "quic-udp-gen.llc"
default_obs_image = "ubuntu-18.04"
...
def run(*, gateway, slicename,
    ...
    rapid_proxy = SshNode(gateway=faraday, hostname=fitname(default_server_rapid),
                          ...
                          verbose=verbose,
                          formatter=TimeHostFormatter())
    cubic_server = SshNode(gateway=faraday, hostname=fitname(default_server_cubic),
                          username="root",
                          verbose=verbose,
                          formatter=TimeHostFormatter())
    obs = SshNode(gateway=faraday, hostname=fitname(default_obs),
                          username="root",
                          verbose=verbose,
                          formatter=TimeHostFormatter())
        ...
        green_light_v1 = [
            ...
            SshJob(
             ...
             SshJob(
                ...
                critical=True,
                verbose=verbose,
                label = f"Load image {default_obs} on {default_obs_image}",
                commands=[
                    Run(f"rhubarbe load {default_obs} -i {default_obs_image}")
                ],
            ),
            SshJob(
                scheduler=scheduler,
    ...
    if quectel:
        ...
        init_quectel_nodes = [
            SshJob(
                scheduler=scheduler,
                required=init_quectel_nodes_v1,
                node=node,
                critical=True,
                verbose=verbose,
                label="Gradle",
